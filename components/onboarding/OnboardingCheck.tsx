...
export default function OnboardingCheck({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function checkOnboardingStatus() {
      ...
      try {
        addDebugLog(`Checking onboarding status for user: ${user.id}`);
        
        // First, try to check the profile using the server API
        addDebugLog('Calling server API to check and create profiles if needed');
        ...
        if (!response.ok) {
          addDebugLog(`Error from server API: ${data.message || 'Unknown error'}`);
        ...
        addDebugLog(`Server API response: ${JSON.stringify(data)}`);
    ...
    // Fallback function to check the profile directly if the API fails
    async function checkProfileDirectly() {
      try {
        // Use the public anon key for client-side requests
        ...
        addDebugLog(`Initializing Supabase client with anon key`);
        const supabase = createClient(supabaseUrl, supabaseAnonKey);

        // Check user profile and onboarding status in one query
        addDebugLog(`Querying user_profiles table for user ${user?.id}`);
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        addDebugLog(`Error in direct profile check: ${errorMessage}`);
